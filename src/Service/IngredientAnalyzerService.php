...
{
    // Список известных ингредиентов с их характеристиками
    ...
    // Список синонимов для ингредиентов
    ...
    private function generateRecommendation(array $safetyPercentages, int $unknownCount, int $totalIngredients): string
    {
        ...
        if ($safetyPercentages['caution'] > 30) {
            return 'Состав содержит значительное количество ингредиентов, которые могут вызывать раздражение у людей с чувствительной кожей.';
        }
        
        if ($safetyPercentages['safe'] > 80) {
            return 'Состав преимущественно безопасен и подходит для большинства типов кожи.';
        }
        
        return 'Состав имеет средний уровень безопасности. Рекомендуем провести тест на аллергическую реакцию перед использованием.';
    }
} 