...
#[AsCommand(
    name: 'app:import-ingredientSynonyms',
    description: 'Импорт ингредиентов и синонимов из CSV-файла',
)]
class ImportIngredientSynonymsCommand extends Command
{
    private EntityManagerInterface $entityManager;
    private IngredientSynonymRepository $ingredientSynonymRepository;
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $synonymIndex = array_search('synonym_name', $headers);

        if ($ingredientIndex === false || $synonymIndex === false) {
        ...
        try {
            while (($row = fgetcsv($file, 0, $delimiter)) !== false) {
                ...
                $synonymsRaw = trim($row[$synonymIndex] ?? '');

                if (empty($ingredientName)) {
                    continue;
                }

                $synonyms = array_map('trim', explode(',', $synonymsRaw));

                $ingredient = $this->ingredientSynonymRepository->findOneBy(['ingredientName' => $ingredientName]);

                if (!$ingredient) {
                    $ingredient = new IngredientSynonym();
                ...
                $ingredient->setSynonymName($synonyms);
